pipeline {
    ...
    stages {
        ...
        stage('Fetch Secrets') {
            steps {
                withCredentials([string(credentialsId: 'infisical-token', variable: 'INFISICAL_TOKEN')]) {
                    bat 'npx -y @infisical/cli export --env="prod" --path="/receitix" --token="%INFISICAL_TOKEN%" > .env'
                }
